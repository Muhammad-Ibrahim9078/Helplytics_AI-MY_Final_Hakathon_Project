...
const Leaderboard = () => {
    ...
    const fetchLeaderboard = async () => {
        try {
            const [rankRes, badgeRes] = await Promise.all([
                ...
                axios.get("http://localhost:8000/api/leaderboard/badges")
            ]);
            if (rankRes.data.success) setRankings(rankRes.data.data);
            if (badgeRes.data.success) setAchievements(badgeRes.data.data);
        } catch (error) {
    ...
    const colors = ["bg-indigo-500", "bg-[#162722]", "bg-orange-500", "bg-teal-600", "bg-rose-500"];
        ...
        <div className="min-h-screen relative overflow-hidden font-sans text-gray-900 flex flex-col" style={{ backgroundColor: '#F4F2EB' }}>
            ...
            <div className="relative z-10 w-full max-w-[1100px] mx-auto px-6 flex-1 flex flex-col pb-12">
                ...
                <div className="w-full bg-[#162722] rounded-[32px] p-8 md:p-12 mb-6 text-white shadow-xl flex flex-col justify-center">
                    <span className="text-[#a4b5ac] text-[11px] font-bold tracking-widest uppercase mb-4 block">LEADERBOARD</span>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col lg:flex-row gap-6">
                        ...
                        <div className="flex-1 bg-[#FAF9F5] rounded-[32px] p-8 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#ebe9e1] flex flex-col">
                            <span className="text-[#1a8570] text-[11px] font-bold tracking-[0.15em] uppercase mb-4 block">TOP HELPERS</span>
                            <h2 className="text-[32px] md:text-[38px] font-bold leading-tight mb-8 tracking-tight text-[#111]">Rankings</h2>
                                ...
                                {rankings.map((user, idx) => (
                                    <div key={user.id} className="bg-white border border-gray-100 rounded-2xl p-5 flex items-center justify-between gap-4 shadow-sm">
                                        <div className="flex items-center gap-4">
                                            <div className={`w-12 h-12 rounded-full ${colors[idx % colors.length]} flex items-center justify-center text-white font-bold text-xs shadow-sm`}>
                                                {user.initials}
                                            </div>
                                            <div className="flex flex-col gap-1">
                                                <h3 className="font-bold text-[15px] text-[#111]">#{user.rank} {user.name}</h3>
                                                <p className="text-[12px] text-gray-400 font-medium">{user.skills}</p>
                                            </div>
                                        </div>
                                        <div className="flex flex-col items-end">
                                            <span className="font-bold text-[15px] text-[#111]">{user.trustScore}</span>
                        ...
                        {/* Achievements */}
                        <div className="flex-1 bg-[#FAF9F5] rounded-[32px] p-8 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#ebe9e1] flex flex-col">
                            <span className="text-[#1a8570] text-[11px] font-bold tracking-[0.15em] uppercase mb-4 block">BADGE SYSTEM</span>
                            <h2 className="text-[32px] md:text-[38px] font-bold leading-tight mb-8 tracking-tight text-[#111]">Trust and achievement</h2>
